...
DIABETES_CATEGORY = {'label': 'Low Saturated Fats', 
                    'id':create_uuid_from_string('Low Saturated Fats'),
                    ...
                    All the items with saturated fat less 5g per 100g portion size is considered as low saturated fat. Though these food items have low saturated fat, it is suggested to have it with caution
...
for item in usfda_data:
    ...
    if guideline_res['tag'] == 'low saturated fat': 
        tag_counter += 1
...
json.dump(final_data, open('../result_data/low_saturated_fats.json', 'w'))
print(("File saved"))
