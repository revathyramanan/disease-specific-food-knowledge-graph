...
# they keywrods from the URL are optimized for search in USFDA
lean_meat = {'keywords':['lean', 'tenderloin', 'pork, loin', 'ham', 'boneless skinless chicken breasts', 'skinless'],
'url':'https://ask.usda.gov/s/article/How-do-I-choose-lean-meat-and-poultry'}

others = {'keywords': ['fish'], 'url':'https://www.niddk.nih.gov/health-information/diabetes/overview/diet-eating-physical-activity'}

# check for all of these labels in category and names
labels = lean_meat['keywords'] + others['keywords'] 



DIABETES_DECISION = {'label':"Recommend", 'id': create_uuid_from_string('Recommend')}
DIABETES_CATEGORY = {'label': 'Animal Protein', 
                    'id':create_uuid_from_string('Animal Protein'),
                    ...
                    'example_items':{'lean meat': lean_meat, 'others':others},
                    'description':"MayoClinic suggests even if you choose lean or extra-lean cuts of beef, don't eat too much. The Dietary Guidelines for Americans recommends that adults eat no more than a total of 26 ounces of meat, poultry and eggs per week. One portion of cooked meat is three ounces. That's about the size of a deck of cards. Read more - https://www.mayoclinic.org/healthy-lifestyle/nutrition-and-healthy-eating/in-depth/cuts-of-beef/art-20043833"
                    }
...
for item in usfda_data:
    ...
    if guideline_res['protein_type'] == 'animal': 
...
json.dump(final_data, open('../result_data/animal_protein.json', 'w'))
print(("File saved"))
